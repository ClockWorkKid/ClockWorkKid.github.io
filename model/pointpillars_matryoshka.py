...
#         # Original Model
#         self.backbone0 = Backbone(in_channel=64, 
#                                  out_channels=[64, 128, 256], 
#                                  layer_nums=[3, 5, 5])
#         self.neck0 = Neck(in_channels=[64, 128, 256], 
#                          upsample_strides=[1, 2, 4], 
#                          out_channels=[128, 128, 128])
#         self.head0 = Head(in_channel=384, n_anchors=2*nclasses, n_classes=nclasses)

#         # Half Channels
#         self.backbone1 = Backbone(in_channel=32, 
#                                  out_channels=[32, 64, 128], 
#                                  layer_nums=[3, 5, 5])
#         self.neck1 = Neck(in_channels=[32, 64, 128], 
#                          upsample_strides=[1, 2, 4], 
#                          out_channels=[64, 64, 64])
#         self.head1 = Head(in_channel=192, n_anchors=2*nclasses, n_classes=nclasses)

#         # Quarter Channels
#         self.backbone2 = Backbone(in_channel=16, 
#                                  out_channels=[16, 32, 64], 
#                                  layer_nums=[3, 5, 5])
#         self.neck2 = Neck(in_channels=[16, 32, 64], 
#                          upsample_strides=[1, 2, 4], 
#                          out_channels=[32, 32, 32])
#         self.head2 = Head(in_channel=96, n_anchors=2*nclasses, n_classes=nclasses)
...
#         #################################################### Full head ####################################################
#         xs0 = self.backbone0(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
#         x0 = self.neck0(xs0)                   # x: (bs, 384, 248, 216)
#         # bbox_cls_pred: (bs, n_anchors*3, 248, 216) 
#         # bbox_pred: (bs, n_anchors*7, 248, 216)
#         # bbox_dir_cls_pred: (bs, n_anchors*2, 248, 216)
#         bbox_cls_pred0, bbox_pred0, bbox_dir_cls_pred0 = self.head0(x0)

#         #################################################### Half head ####################################################
#         xs1 = self.backbone1(pillar_features[:, :32, :, :]) 
#         x1 = self.neck1(xs1)                  
#         bbox_cls_pred1, bbox_pred1, bbox_dir_cls_pred1 = self.head1(x1)

#         #################################################### Qurt head ####################################################
#         xs2 = self.backbone2(pillar_features[:, :16, :, :]) 
#         x2 = self.neck2(xs2) 
#         bbox_cls_pred2, bbox_pred2, bbox_dir_cls_pred2 = self.head2(x2)
...
#         # anchors
#         device = bbox_cls_pred0.device
#         feature_map_size = torch.tensor(list(bbox_cls_pred0.size()[-2:]), device=device)
...
#             return bbox_cls_pred0, bbox_pred0, bbox_dir_cls_pred0, \
#                 bbox_cls_pred1, bbox_pred1, bbox_dir_cls_pred1, \
#                     bbox_cls_pred2, bbox_pred2, bbox_dir_cls_pred2, \
#                         anchor_target_dict
        
#         elif (mode == 'val'):
#             results0 = self.get_predicted_bboxes(bbox_cls_pred=bbox_cls_pred0, 
#                                                 bbox_pred=bbox_pred0, 
#                                                 bbox_dir_cls_pred=bbox_dir_cls_pred0, 
#                                                 batched_anchors=batched_anchors)
#             results1 = self.get_predicted_bboxes(bbox_cls_pred=bbox_cls_pred1, 
#                                                 bbox_pred=bbox_pred1, 
#                                                 bbox_dir_cls_pred=bbox_dir_cls_pred1, 
#                                                 batched_anchors=batched_anchors)
#             results2 = self.get_predicted_bboxes(bbox_cls_pred=bbox_cls_pred2, 
#                                                 bbox_pred=bbox_pred2, 
#                                                 bbox_dir_cls_pred=bbox_dir_cls_pred2, 
#                                                 batched_anchors=batched_anchors)
#             return results0, results1, results2, pillar_features        
...
class PointPillars(nn.Module):
    def __init__(self,
        ...
        self.backbone0 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.backbone1 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.backbone2 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.backbone3 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.backbone4 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.backbone5 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.backbone6 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.backbone7 = Backbone(in_channel=64, out_channels=[8, 16, 32], layer_nums=[3, 5, 5])
        self.neck0 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
        self.neck1 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
        self.neck2 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
        self.neck3 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
        self.neck4 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
        self.neck5 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
        self.neck6 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
        self.neck7 = Neck(in_channels=[8, 16, 32], upsample_strides=[1, 2, 4], out_channels=[16, 16, 16])
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        ...
        #################################################### Full head ####################################################
        xs0 = self.backbone0(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs1 = self.backbone1(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs2 = self.backbone2(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs3 = self.backbone3(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs4 = self.backbone4(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs5 = self.backbone5(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs6 = self.backbone6(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs7 = self.backbone7(pillar_features) # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs0 = self.neck0(xs0)
        xs1 = self.neck0(xs1)
        xs2 = self.neck0(xs2)
        xs3 = self.neck0(xs3)
        ...
        x0 = torch.cat([xs0, xs1, xs2, xs3, xs4, xs5, xs6, xs7], dim=1)
        # bbox_cls_pred: (bs, n_anchors*3, 248, 216) 
        # bbox_pred: (bs, n_anchors*7, 248, 216)
        # bbox_dir_cls_pred: (bs, n_anchors*2, 248, 216)
        bbox_cls_pred0, bbox_pred0, bbox_dir_cls_pred0 = self.head(x0)
        ...
        if mode == 'train':
            ...
            return bbox_cls_pred0, bbox_pred0, bbox_dir_cls_pred0, anchor_target_dict
